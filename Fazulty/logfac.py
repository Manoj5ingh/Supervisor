...
def chckdata():
   ...
   user=username.get()
   code=password.get()
   cur.execute('SELECT * FROM reg_fac WHERE uid = ? AND password = ?', (user, code))
   if cur.fetchall():
...
w = Label(top, text=" Supervisor Login", font=("Monotype Corsiva", 40))
w.place(x=500,y=150)
